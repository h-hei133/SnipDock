...
当前版本：**v0.5.1**。这个版本聚焦 UI 产品化收口、主题资源整理、Markdown 预览样式和图标接入，同时继续保持本地优先和轻量定位。
...
- 安装包：运行 `SnipDock-Setup-v0.5.1.exe`，按照安装向导完成安装。
- Zip / 便携包：解压后直接运行：
...
- v0.5.1：UI 产品化收口、主题资源整理、Markdown 样式和图标接入
- v0.5.0：批量管理与重复检测
